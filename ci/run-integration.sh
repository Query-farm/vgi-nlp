...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-nlp
...
# ATTACH (`${VGI_NLP_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run nlp_worker.py`) — the extension
...
#                        `uv run --python 3.13 <repo>/nlp_worker.py`.
# Optional:
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_NLP_WORKER (the LOCATION) and, for the out-of-band
...
case "$TRANSPORT" in
  subprocess)
    export VGI_NLP_WORKER="$WORKER_CMD"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/nlp-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/nlp-unix-server.log"
    ...
    export VGI_NLP_WORKER="unix://$SOCK"
    ;;
